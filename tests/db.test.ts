...
describe("createSchema", () => {
	...
	it("creates FTS5 virtual tables", () => {
		const db = makeTestDb();
		const vtables = (
			db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name LIKE '%_fts'").all()
		) as Array<{ name: string }>;
...
describe("FTS5 triggers", () => {
	it("indexes sections on insert", () => {
		const db = makeTestDb();
		db.prepare("INSERT INTO sections(name, content) VALUES ('main', 'calf injury achilles rules')").run();
		const results = db.prepare("SELECT name FROM sections_fts WHERE sections_fts MATCH 'achilles'").all();
